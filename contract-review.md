...
**关键审查要素：**
- 赔偿上限金额（固定金额、费用倍数，或未设上限）
- 上限是否适用于双方（互惠）或存在不对称安排
- 赔偿上限的例外情形（需重点审查是否过宽或规避上限）
- 是否明确约定不承担超出违约时可预见范围的损失
- 是否存在对一方明显不利的责任排除或限制
- 责任上限的计算方式（单次索赔 / 按年度 / 累计总额）
- 是否明确**实际损失+合理费用（如律师费）**的范围
- 是否包含依法不得限制或免除的责任的例外条款
